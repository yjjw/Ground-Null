...
bool BackGround::init() {
	...
	float wr = backGround.width;
	float hr = backGround.height;
...
void BackGround::set_player_dead() {
	m_dead_time = glfwGetTime();
}

void BackGround::reset_player_dead_time() {
	m_dead_time = -1;
}

float BackGround::get_player_dead_time() const {
	return glfwGetTime() - m_dead_time;
}

int BackGround::getEntityType()
{
	return ET_background;
}

void BackGround::handleCollision(Renderable * r1, Renderable * r2, int r1EntityType, int r2EntityType)
{
}

void BackGround::draw(const mat3& projection, mat3& view_2d) {
	transform_begin();
