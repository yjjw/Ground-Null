...
bool FrontGround::init() {
	...
	float wr = frontGround.width;
	float hr = frontGround.height;
...
void FrontGround::set_player_dead() {
	m_dead_time = glfwGetTime();
}

void FrontGround::reset_player_dead_time() {
	m_dead_time = -1;
}

float FrontGround::get_player_dead_time() const {
	return glfwGetTime() - m_dead_time;
}

int FrontGround::getEntityType()
{
	return ET_frontground;
}

void FrontGround::handleCollision(Renderable * r1, Renderable * r2, int r1EntityType, int r2EntityType)
{
}

void FrontGround::draw(const mat3& projection, mat3& view_2d) {
	transform_begin();
	...
	glBindTexture(GL_TEXTURE_2D, frontGround.id);
