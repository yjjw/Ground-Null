...
Texture Knight::run_Textures[];
Texture Knight::attack_Textures[];

bool Knight::newInit(Player* player, std::vector<Fire>* m_fire, HealthPoints* heathp)
{
	init(knight_run, player, m_fire, heathp);
	exp = 20;
	hp = 15;
	max_step -= 1.0f;
	type = 2;
	...
	knightBodyDef.type = b2_dynamicBody;
	float32 x = m_position.x * pixeltob2;
	float32 y = m_position.y * pixeltob2;
	knightBodyDef.position.Set(x, y);
	knightBodyDef.angle = 0;
	...
	knightShape.SetAsBox(width / 2, height / 2);
	knightFixtureDef.shape = &knightShape;
	knightFixtureDef.density = 1;
	knightFixtureDef.friction = 0.3;

	return true;
}

int Knight::getEntityType()
{
	return ET_kight;
}

void Knight::handleCollision(Renderable * r1, Renderable * r2, int r1EntityType, int r2EntityType)
{
...
void Knight::draw(const mat3& projection, mat3& view_2d)
{
