...
Texture KeyUI::KeyUI_textures[5];

bool KeyUI::init(Player* p)
{
...
void KeyUI::update(float ms)
{
	ui_texture = KeyUI_textures[player->get_keys()];
}

void KeyUI::draw(const mat3& projection, mat3& view_2d)
{
...
vec2 KeyUI::get_position()const
{
	return m_position;
}

void KeyUI::set_position(vec2 position)
{
	m_position = position;
}

int KeyUI::getEntityType()
{
	return ET_keyui;
}

void KeyUI::handleCollision(Renderable * r1, Renderable * r2, int r1EntityType, int r2EntityType)
{
}
