...
float dot(vec4 l, vec4 r) {
	return l.x*r.x + l.y*r.y + l.z*r.z + l.a*r.a;
}
...
{
	float m = sqrtf(dot(v, v));
	return { v.x / m, v.y / m, v.z/m, v.a/m };
}
...
/*bool load_from_files(char* paths[], Texture texture_arrays[], int length)
{
	...
	for (int i = 0; i < length; i++) {
		...
		stbi_uc* data = stbi_load(paths[i], &(texture_arrays[i].width), &(texture_arrays[i].height), NULL, 4);
		if (data == NULL)
		return false;
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (texture_arrays[i].width), (texture_arrays[i].height), 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		stbi_image_free(data);
	}
...
bool Texture::create_from_screen(GLFWwindow const * const window, const char* path) {
	if (path == nullptr)
		return false;
...
void Renderable::transform_mirror()
{
	mat3 T = { { -1.f, 0.f, 0.f },{ 0.f, 1.f, 0.f },{ 0.f, 0.f, 1.f } };
	transform = mul(transform, T);
}

void Renderable::transform_updown()
{
	mat3 T = { { 1.f, 0.f, 0.f },{ 0.f, -1.f, 0.f },{ 0.f, 0.f, 1.f } };
	transform = mul(transform, T);
}
