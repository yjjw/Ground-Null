...
Texture Start::start_textures[2];

bool Start::init()
{
	char texPath[] = starter;
	char texName[] = "0.png";
	char buffer[256];
...
int Start::getEntityType()
{
	return ET_start;
}

void Start::handleCollision(Renderable * r1, Renderable * r2, int r1EntityType, int r2EntityType)
{
}
...
void Start::increase_pages() {
	if (page > 0) page--;
}

void Start::decrease_pages() {
	if (page < 1) page++;
}

void Start::update(float ms)
{
}

void Start::draw(const mat3& projection, mat3& view_2d)
{
	...
	glBindTexture(GL_TEXTURE_2D, start_textures[page].id);
...
vec2 Start::get_position()const
{
	return m_position;
}

void Start::set_position(vec2 position)
{
	m_position = position;
}
